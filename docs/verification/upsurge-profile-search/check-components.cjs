...
(async () => {
 const result = await esbuild.build({
  stdin: { contents: `import React from 'react'; import {createRoot} from 'react-dom/client'; import {UpsurgeSearchDialog} from './components/public/upsurge-search-dialog'; import {UpsurgeProfileForm} from './components/public/upsurge-profile-form'; import styles from './components/public/upsurge-profile.module.css';
 ...
 try{
  ...
  await page.setViewportSize({width:390,height:844});await page.screenshot({path:'docs/verification/upsurge-profile-search/search-mobile.png'});
  ...
  await page.setViewportSize({width:390,height:844});await page.screenshot({path:'docs/verification/upsurge-profile-search/profile-mobile.png',fullPage:true});assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true);
  assert.deepEqual(errors,[]);console.log('PASS: search focus/filter/empty/submit/category/Escape/mobile; profile required-name/save/error/gender/image/email/mobile. Supabase and router mocked.');
